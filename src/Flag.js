  ...
  return (
    <>
      <div className='app'>
        ...
        <input style={{ width: '50%', padding: '10px', borderRadius: '20PX', paddingLeft: '50px' }} type='text' placeholder='Search Here'
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '3fr 3fr 3fr 3fr', columnGap: '20px', rowGap: '20px' }}>
          {
            myArray1.filter(([key, val]) => {
              if (serchTerm == '') {
                return val
              } else if (val.toLowerCase().includes(serchTerm.toLowerCase())) {
                return val
              }
            }).map(([key, val]) => {
              return (
